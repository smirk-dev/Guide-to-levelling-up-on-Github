...
import { RankTier, RPGStats } from '@/types/database';
 ...
 */
export function calculateRPGStats(stats: GitHubStats): RPGStats {
  return {
    health: Math.min(100, stats.totalCommits * 2), // Max 100
    mana: Math.min(100, (stats.totalIssues + stats.totalReviews) * 5), // Max 100
    strength: Math.min(100, stats.totalPRs * 3), // Max 100
    charisma: Math.min(100, stats.totalStars * 2), // Max 100
    wisdom: Math.min(100, (stats.totalIssues + stats.totalReviews) * 4), // Max 100
  };
}
